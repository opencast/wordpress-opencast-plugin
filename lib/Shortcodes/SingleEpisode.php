...
class SingleEpisode
{
    ...
    private function set_single_episode_object($attr) {
        ...
        $this->wp_id = (isset($attr['wp_id']) && !empty($attr['wp_id'])) ? esc_html($attr['wp_id']) : '';
        $this->class = (isset($attr['class']) && !empty($attr['class'])) ? esc_html($attr['class']) : '';
        ...
        if (isset($single_episode_list[$this->wp_id])) {
            ...
            if (isset($_GET['oc_id']) && isset($_GET['wp_id']) && $this->wp_id == filter_var($_GET['wp_id'], FILTER_SANITIZE_STRING)) {
                $this->oc_id = filter_var($_GET['oc_id'], FILTER_SANITIZE_STRING);
                $this->usepermissions = true;
    ...
    private function render_selectable_episodes_list(){
        ...
        $selectable_episodes_list = $this->generate_default_style();
        ...
        if ($episodes = $this->get_episodes()) {
            ...
            foreach ($episodes as $episode) {
                ...
                $select_link = add_query_arg($current_page_query_vars , $current_page_url );
            ...
            $selectable_episodes_list .= "<div class='" . implode(' ', $class) . "'>";
            ...
            $selectable_episodes_list .= "<div class='search' style='display: none;'><input type='text' placeholder='Search' class='oc-list-search-text'/></div>";

            foreach ($offsets as $index => $offset) {
                $selectable_episodes_list .= "<div class='offset " . (($index == 0) ? 'active' : '') . "' data-index='" . ($index + 1) . "'>";

                foreach ($offset as $rows) {
                    $selectable_episodes_list .= "<div class='row'>" . implode('', $rows) . "</div>";
                }
    ...
    private function render_single_episode(){
        ...
        $single_episode_container .= "<div class='oc-player-container {$this->class}'>";
        $opencast_options = $this->opencast_options;
        ...
        $src = rtrim($endpoint, '/') . '/paella/ui/embed.html?id=' . $this->oc_id;
        $safarihelper = PLUGIN_DIR_URL . 'safariHelper.php';
        if ($this->usepermissions) {
    ...
    private function generate_default_style() {
        return "<style>".
                    file_get_contents(PLUGIN_DIR . 'src/css/inlines/single-episode.css').
                "</style>";
    }
}
